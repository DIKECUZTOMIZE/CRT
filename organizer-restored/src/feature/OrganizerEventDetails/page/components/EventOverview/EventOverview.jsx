...
export const EventOverview = ({ meta, overview }) => {
  ...
  const mainStats = [
    ...
    },
    {
      ...
      show: Boolean(data.statusReason && (data.status === "cancelled" || data.status === "postponed")),
    },
    {
      icon: Users,
          ...
          {mainStats.map((item, idx) => {
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex min-w-0 items-center">
                      <span
                        className={`inline-flex max-w-full items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[11px] font-extrabold capitalize ${statusStyle.badgeClass}`}
                      >
                        <span
            ...
            return (
              <div
                key={idx}
                title={item.value}
                className="group relative flex min-w-0 items-center gap-3 overflow-hidden rounded-xl border border-slate-800/70 bg-slate-950/70 p-3 transition-all duration-300 hover:border-emerald-500/30 hover:bg-slate-950"
              >
                <div className="absolute left-1/2 top-full z-20 mt-2 w-[300px] max-w-[80vw] -translate-x-1/2 translate-y-2 rounded-xl border border-emerald-500/30 bg-slate-950/95 p-2.5 text-left text-slate-100 opacity-0 shadow-2xl shadow-slate-950/60 ring-1 ring-slate-800/80 transition-all duration-200 group-hover:translate-y-0 group-hover:opacity-100 group-hover:visible invisible">
                  <div className="mb-1 text-[9px] font-bold uppercase tracking-[0.18em] text-emerald-400">
                    {item.label}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 min-w-0 max-w-full overflow-hidden text-ellipsis line-clamp-2 break-words text-xs font-bold text-slate-200 [overflow-wrap:anywhere] group-hover:text-white">
                    {item.value}
                  </p>
                </div>
              </div>
            ...
            {scheduleItems.map((item, idx) => {
              ...
              return (
                <div
                  ...
                  className="group relative min-w-0 overflow-hidden rounded-xl border border-slate-800/70 bg-slate-950/70 p-3.5 transition-all duration-300 hover:-translate-y-0.5 hover:border-emerald-500/30 hover:bg-slate-950"
                >
                  <div className="absolute left-1/2 top-full z-20 mt-2 w-[300px] max-w-[80vw] -translate-x-1/2 translate-y-2 rounded-xl border border-emerald-500/30 bg-slate-950/95 p-2.5 text-left text-slate-100 opacity-0 shadow-2xl shadow-slate-950/60 ring-1 ring-slate-800/80 transition-all duration-200 group-hover:translate-y-0 group-hover:opacity-100 group-hover:visible invisible">
                    <div className="mb-1 text-[9px] font-bold uppercase tracking-[0.18em] text-emerald-400">
                      {item.label}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 min-w-0 max-w-full overflow-hidden text-ellipsis line-clamp-2 break-words text-xs font-extrabold tracking-tight text-slate-200 [overflow-wrap:anywhere] group-hover:text-white">
                        {item.date}
                      </p>
